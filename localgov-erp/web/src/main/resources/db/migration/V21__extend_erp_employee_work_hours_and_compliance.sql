-- Extend erp_employee with compliance/work-schedule fields adapted from requested employees table
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'chk_erp_employee_division'
    ) THEN
        ALTER TABLE erp_employee
            ADD CONSTRAINT chk_erp_employee_division
    ...
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'chk_erp_employee_role_category'
    ) THEN
        ALTER TABLE erp_employee
            ADD CONSTRAINT chk_erp_employee_role_category
    ...
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'chk_erp_employee_health_break_type'
    ) THEN
        ALTER TABLE erp_employee
            ADD CONSTRAINT chk_erp_employee_health_break_type
    ...
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'chk_erp_employee_nursing_breaks_entitled'
    ) THEN
        ALTER TABLE erp_employee
            ADD CONSTRAINT chk_erp_employee_nursing_breaks_entitled
    ...
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'chk_erp_employee_gender'
    ) THEN
        ALTER TABLE erp_employee
            ADD CONSTRAINT chk_erp_employee_gender
    ...
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'uq_erp_employee_national_id'
    ) THEN
        ALTER TABLE erp_employee
            ADD CONSTRAINT uq_erp_employee_national_id UNIQUE (national_id);
    END IF;

    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'uq_erp_employee_qr_code'
    ) THEN
        ALTER TABLE erp_employee
            ADD CONSTRAINT uq_erp_employee_qr_code UNIQUE (qr_code);
    END IF;

    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'fk_erp_employee_supervisor'
    ) THEN
        ALTER TABLE erp_employee
            ADD CONSTRAINT fk_erp_employee_supervisor
            FOREIGN KEY (supervisor_id) REFERENCES erp_employee(id);
    END IF;

    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'fk_erp_employee_hod'
    ) THEN
        ALTER TABLE erp_employee
            ADD CONSTRAINT fk_erp_employee_hod
            FOREIGN KEY (hod_id) REFERENCES erp_employee(id);
    END IF;
END $$;

CREATE OR REPLACE FUNCTION fn_set_erp_employee_work_hours()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
DROP TRIGGER IF EXISTS trg_set_erp_employee_work_hours ON erp_employee;

CREATE TRIGGER trg_set_erp_employee_work_hours
BEFORE INSERT OR UPDATE ON erp_employee
FOR EACH ROW
EXECUTE FUNCTION fn_set_erp_employee_work_hours();
