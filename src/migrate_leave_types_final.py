...
for row in data:
    ...
    """, (
        row[0],  # leave_type_id
        row[1],  # leave_type_code
        row[2],  # leave_type_name
        row[3],  # description
        to_bool(row[4]),   # requires_approval
        to_bool(row[5]),   # is_paid
        to_bool(row[6]),   # is_cumulative
        row[7],  # max_days_per_month
        row[8],  # max_days_per_year
        row[9],  # applicable_to
        to_bool(row[10]),  # requires_supervisor_notification
        to_bool(row[11]),  # requires_hr_notification
        row[12]  # created_at
    ))
...
# Verify
cur.execute("SELECT leave_type_code, leave_type_name, applicable_to, requires_approval, is_paid FROM leave_types;")
results = cur.fetchall()
print("\n📋 Migrated leave types:")
for r in results:
    print(f"   - {r[0]}: {r[1]} (Applicable: {r[2]}, Approval required: {r[3]}, Paid: {r[4]})")
