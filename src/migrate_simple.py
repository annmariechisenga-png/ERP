...
if rows:
    ...
    cur.executemany(f"INSERT INTO employees ({','.join(cols)}) VALUES ({placeholders})", rows)
...
if rows:
    cols = [desc[0] for desc in sqlite_cur.description]
    placeholders = ','.join(['%s'] * len(cols))
    cur.executemany(f"INSERT INTO positions ({','.join(cols)}) VALUES ({placeholders})", rows)
    print(f"  Inserted {len(rows)} positions")
...
print(f"✅ Positions: {cur.fetchone()[0]} rows")
